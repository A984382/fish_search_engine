...
public class AccessTracker_p{
  ...
  */
  private static String getRelativePathToThis(final String path){
    return (path!=null && path.startsWith("/")) ? path.substring(1,path.length()) : null;
  }
  ...
  public static serverObjects respond(final RequestHeader header,final serverObjects post,final serverSwitch env){
    ...
    if(page==0){
      ...
      entCount=0;
      try{
        for(;(entCount < maxCount) && (i.hasNext());++entCount){
          host=i.next();
      ...
      entCount=0;
      Collection<Track> access=serverAccessTracker.accessTrack(host);
      Track entry;
      if(host.length()>0 && access!=null){
        try{
          final Iterator<Track> ii=listclone(access).iterator();
          for(;ii.hasNext();++entCount){
            entry=ii.next();
            prop.putHTML("page_list_"+entCount+"_host",host);
    ...
    QueryParams query;
    long qcountSum=0, rcountSum=0, tcountSum=0, rcount=0, utimeSum=0, stimeSum=0, rtimeSum=0, utimeSum1=0, stimeSum1=0, rtimeSum1=0;
    int m=0;
    ...
    for(;ai.hasNext();++m){
      ...
      prop.putNum("page_list_"+m+"_resultcount",0);
    ...
    if(page==2 && AccessTracker.getDumpFile()!=null && AccessTracker.getDumpFile().exists()){
      ...
      if(topicNavigator==null || topicNavigator.isEmpty()){
        ...
        // topics navigator
        final int TOPWORDS_MAXCOUNT=25, TOPWORDS_MINSIZE=9, TOPWORDS_MAXSIZE = 24;
        int count, i=0;
        ...
        for(;i<TOPWORDS_MAXCOUNT && navigatorIterator.hasNext();++i){
          ...
          short fontsize=TOPWORDS_MINSIZE+(TOPWORDS_MAXSIZE-TOPWORDS_MINSIZE)*(count / maxcount);
      ...
      try{
        for(;(m<maxCount) && (i.hasNext());++m){
          ...
          final int qph=handles.tailSet(Long.valueOf(System.currentTimeMillis()-1000*3600)).size(); qphSum+=qph;
